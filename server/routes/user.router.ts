...
router.post("/register", async (req, res, next) => {
  ...
  try {
    ...
    const newUser = await client.query(queryText, valueText);
    const siteText = `INSERT INTO "users_sites" ("users_id", "sites_id") VALUES ($1, 1)`;
    await client.query(siteText, [newUser.rows[0].id]);
